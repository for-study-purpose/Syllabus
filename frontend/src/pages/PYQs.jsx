...
export default function PYQs() {
  ...
  // Static PYQs from RTDB
  const { docs: staticDocs } = useFirestoreQuery('staticFiles', [['category', '==', 'pyq']])
  const { docs: community } = useFirestoreQuery('submissions', [
    ['category', '==', 'pyq'],
    ['status', '==', 'approved'],
  ])

  // Build ordered group list from flat static docs
  const groups = useMemo(() => {
    ...
    sorted.forEach(doc => {
      if (!map[doc.subject]) {
        const meta = SUBJECT_META[doc.subject] ?? { label: doc.subject, accent: 'cyan' }
        map[doc.subject] = {
          subject: meta.label,
          short: doc.subject,
      ...
      map[doc.subject].items.push({ id: doc.id, title: doc.title, file: getViewUrl(doc.fileId), badge: doc.badge })
    })
    return Object.values(map).sort((a, b) => a.groupOrder - b.groupOrder)
  }, [staticDocs])
  ...
  function matchesByFilter(item, groupShort, groupLabel) {
    if (!needle) return true
    const fields = {
      title: [item.title],
      subject: [item.subject, groupShort, groupLabel],
      unit: [item.unit, item.badge],
      all: [item.title, item.subject, groupShort, groupLabel, item.unit, item.badge],
    }
  ...
  const hasVisibleData =
    visible.some(g => {
      const meta = SUBJECT_META[g.short] ?? { label: g.subject }
      const staticMatches = g.items.some(item => matchesByFilter(item, g.short, meta.label))
      const communityMatches = community.some(s => s.subject === g.short && matchesByFilter(s, g.short, meta.label))
      return staticMatches || communityMatches
    }) ||
    (active === 'All'
      ? communitySubjects.some(code => {
          const meta = SUBJECT_META[code] ?? { label: code }
          return community.some(s => s.subject === code && matchesByFilter(s, code, meta.label))
        })
      : communitySubjects.includes(active) && community.some(s => s.subject === active && matchesByFilter(s, active, active)))

  return (
    <Layout
      label="Exam Papers"
      title="Previous Year Questions"
      subtitle="Past exam papers. Open a file directly or jump to a specific page."
    >
      <div className="flex items-center justify-between gap-3 mb-5">
        <h3 className="text-sm font-semibold text-slate-300">Filter papers</h3>
      ...
      <div className="mb-7 space-y-3">
        ...
        {communitySubjects.map(code => {
          const meta = SUBJECT_META[code] ?? { label: code, accent: 'cyan' }
          return (
            <FilterButton
              key={code}
              active={active === code}
              onClick={() => setActive(code)}
              activeClass={ACCENT_ACTIVE[meta.accent] || ACCENT_ACTIVE.cyan}
            >
              {code} — {meta.label}
            </FilterButton>
          )
      ...
      {!hasAnyData && (
        <EmptyState
          icon={<FileIcon />}
          message="No question papers available yet"
          subtext="Approved question papers will appear here after review."
        />
      )}
      ...
      {hasAnyData && !hasVisibleData && (
        <EmptyState
          icon={<FileIcon />}
          message={search ? 'No exam papers match your search' : 'No question papers for this subject'}
      ...
      {hasVisibleData && visible.map(g => {
        const staticItems = g.items.filter(q => matchesByFilter(q, g.short, g.subject))
        const communityItems = community.filter(
          s => s.subject === g.short && matchesByFilter(s, g.short, g.subject)
        )
        if (staticItems.length === 0 && communityItems.length === 0) return null
        ...
        return (
          <section key={g.subject} className="mb-8 animate-fade-in">
            ...
            {staticItems.map((q, i) => (
              <PDFCard key={q.id} title={q.title} file={q.file} badge={q.badge} accent={g.accent} index={i} />
            ))}
            {communityItems.map(s => (
              <PDFCard
                key={s.id}
                title={s.title || `${g.short} Exam${s.unit ? ' — ' + s.unit : ''}`}
                file={getViewUrl(s.fileId)}
                badge={s.unit || 'Community'}
                accent={g.accent}
              />
            ))}
          </section>
      ...
      {/* Dynamic sections for community PYQs on subjects not in static groups */}
      {hasVisibleData && (active === 'All' || communitySubjects.includes(active)) && (() => {
        ...
        return keys.map(code => {
          const papers = bySub[code].filter(s => matchesByFilter(s, code, code))
          if (papers.length === 0) return null
          const meta = SUBJECT_META[code] ?? { label: code, accent: 'cyan', sem: '' }
          ...
          return (
            <section key={code} className="mb-8 animate-fade-in">
              ...
              {papers.map((s, i) => (
                <PDFCard
                  key={s.id}
                  title={s.title || `${meta.label} Exam${s.unit ? ' — ' + s.unit : ''}`}
                  file={getViewUrl(s.fileId)}
                  badge={s.unit || semShort || 'Community'}
                  accent={meta.accent}
                  index={i}
                />
              ))}
